...
describe('Queue Scheduling Capabilities', function () {
  ...
  it('should be able to execute jobs every after specific time interval',
    function (done) {
  ...
  describe('Queue CRUD Capabilities', function () {
    ...
    it('should be able to save job data', function (done) {
      Queue
        ._saveJobData(jobDataKey, jobData, function (error,
          _jobData) {
      ...
      async.waterfall([
        ...
        function removeJob(job, next) {
          Queue.remove(job, function (error, response) {
            ...
            expect(response.removedJobInstance.type).to.be.equal(
              'unique_every');
            expect(response.removedJobInstance.data.unique)
              .to.be.equal('every_mail');
            next(error, response);
          });
        }
      ], done);
  ...
  describe('Queue `nextRun` Capabilities', function () {
    ...
    it('should be able to compute next run from human interval',
      function (done) {
        var expectedNextRunTime = new Date(lastRun.valueOf());
        expectedNextRunTime.setMinutes(expectedNextRunTime.getMinutes() +
          5);
      ...
      function (done) {
        ...
        Queue._computeNextRunTime({
          ...
          if (error) {
            ...
            expect(nextRun.getSeconds()).to.equal(lastRun.getSeconds() +
              1);
            done();
          }
    ...
    it(
      'should throw `Invalid reccur interval` if interval is not human interval or cron interval',
      function (done) {
        Queue._computeNextRunTime({
          reccurInterval: 'abcd'
        }, function (error, nextRun) {
          expect(error.message).to.equal(
            'Invalid reccur interval');
          done(null, nextRun);
        });
      });
  ...
  it('should be able to enable key expriration notifications', function (
    done) {
